...
class AVLTree:
    ...
    # Functions added for calculating the hamming distance

    def get_most_similar(self, key):

        if self.root is not None:
            global lowest_hamming_distance
            global most_similar_node
            global most_similar_node_key

            most_similar_node = self.root
            most_similar_node_key = self.root.key
            lowest_hamming_distance = imagehash.hex_to_hash(key) - imagehash.hex_to_hash(self.root.key)

            return self._get_most_similar(key, self.root)
        else:
            return 'Es wurde kein Dataset gefunden'

    def _get_most_similar(self, key, cur_node):
        global lowest_hamming_distance
        global most_similar_node
        global most_similar_node_key

        if imagehash.hex_to_hash(key) - imagehash.hex_to_hash(cur_node.key) < imagehash.hex_to_hash(key) - \
                imagehash.hex_to_hash(most_similar_node_key) and key != cur_node.key:
            most_similar_node = cur_node
            lowest_hamming_distance = imagehash.hex_to_hash(key) - imagehash.hex_to_hash(cur_node.key)

        if key == cur_node.key:
            if cur_node.left_child is not None:

                if imagehash.hex_to_hash(key) - imagehash.hex_to_hash(cur_node.left_child.key) < \
                        imagehash.hex_to_hash(key) - imagehash.hex_to_hash(most_similar_node_key):
                    most_similar_node = cur_node.left_child
                    lowest_hamming_distance = imagehash.hex_to_hash(key) - imagehash.hex_to_hash(
                        cur_node.left_child.key)

            if cur_node.right_child is not None:

                if imagehash.hex_to_hash(key) - imagehash.hex_to_hash(cur_node.right_child.key) < \
                        imagehash.hex_to_hash(key) - imagehash.hex_to_hash(most_similar_node_key):
            ...
            return self._get_most_similar(key, cur_node.left_child)
        elif key > cur_node.key and cur_node.right_child is not None:
            return self._get_most_similar(key, cur_node.right_child)
        print(f'{most_similar_node.getPaths()}')
